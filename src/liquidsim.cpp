...
void LiquidSim::update_simulation() {
	...
	for(it = cells.begin(); it != cells.end(); it++) {
		...
		if(world->get_cell(cell->x, cell->y + 1) == -1) {
			...
			float flow = get_vertical_flow_value(cell->new_liquid_amount, bottom_liquid_amount) - bottom_liquid_amount;
			...
			if(flow != 0.0f) {
				if(bottom_cell == nullptr) {
					...
					cells_positions.insert(std::make_pair(bottom_cell->uid, bottom_cell));
				}
			...
			if(left_liquid_amount < cell->liquid_amount) {
				...
				if(flow != 0.0f) {
					if(left_cell == nullptr) {
						...
						cells_positions.insert(std::make_pair(left_cell->uid, left_cell));
					}
		...
		if(world->get_cell(cell->x + 1, cell->y) == -1) {
			...
			if(right_liquid_amount < cell->liquid_amount) {
				float flow = (cell->new_liquid_amount - right_liquid_amount) / 3;
				if(flow > min_flow_value) {
					flow *= flow_speed;
				}
				...
				if(flow != 0.0f) {
					if(right_cell == nullptr) {
						...
						cells_positions.insert(std::make_pair(right_cell->uid, right_cell));
					}
		...
		if(world->get_cell(cell->x, cell->y - 1) == -1) {
			...
			float flow = cell->new_liquid_amount - get_vertical_flow_value(cell->new_liquid_amount, top_liquid_amount);
			if(flow > min_flow_value) {
				flow *= flow_speed;
			}
			...
			if(flow != 0.0f) {
				if(top_cell == nullptr) {
					...
					cells_positions.insert(std::make_pair(top_cell->uid, top_cell));
				}
...
int LiquidSim::cleanup_empty_cells(int limit) {
	...
	for(it = cells.begin(); it != cells.end();) {
		...
		if(cell->liquid_amount == 0.0f && cell->iteration_since_last_change > 10) {
			it = cells.erase(it);
...
Cell *LiquidSim::create_cell(int x, int y, float amount) {
	Node2D *node = godot::Object::cast_to<godot::Node2D>(liquid_scene->instance());
	Cell *cell = new Cell(node, x, y, amount);
	add_child(node);
	return cell;
}

float LiquidSim::get_vertical_flow_value(float src_liquid_amount, float dst_liquid_amount) {
	...
	if(sum <= max_liquid_value) {
		...
		value = (max_liquid_value * max_liquid_value + sum * max_liquid_compression) / (max_liquid_value + max_liquid_compression);
	} else {
...
void LiquidSim::refresh_cell_sprite(Cell *cell) {
	...
	Cell *top_cell = get_cell_by_position(cell->x, cell->y - 1);
	if(top_cell != nullptr && (top_cell->liquid_amount >= min_liquid_value || top_cell->dir_bottom)) {
		scale = 1.0f;
	}
